...
# -*- coding: utf-8 -*-
"""
Step4: 曲线+变速 (combined) — 动态平台降落评估
================================================
船舶以正弦/圆周曲线上叠加随机变速 (combined), 无人机端到端自主降落。

用法:
  python step4_env.py --test_episodes 100 --curve sine --v_min 0.4 --v_max 1.0
  python step4_env.py --test_episodes 100 --curve circle --v_min 0.4 --v_max 1.0
"""
...
def main():
    parser = argparse.ArgumentParser(description="Step4: 曲线+变速 (combined) 降落评估")
    # ---- 评估控制 ----
    ...
    # ==================== 打印配置 ====================
    print("=" * 60)
    print(f"  Step4 曲线+变速 (combined:{args.curve}) 降落评估")
    print("=" * 60)
    ...
    try:
        ...
        # combined: 先配置底层曲线, 再叠加随机变速
        if args.curve == 'sine':
            controller.set_mode_sine(v=1.0, amp=2.0, wavelen=8.0, vx=args.vx, vy=args.vy)
        else:
            controller.set_mode_circle(radius=3.0, period=40.0, vx=args.vx, vy=args.vy)
        controller.set_mode_combined(args.curve, (args.v_min, args.v_max), args.seed)
        print(f"  ✓ ShipMotionController 就绪: mode=combined "
              f"curve={args.curve} range=[{args.v_min:.1f},{args.v_max:.1f}]")
        ...
        print(f"  模型:           {args.ckpt_dir}  step={args.load_step}")
        print(f"  模式:           combined:{args.curve}  "
