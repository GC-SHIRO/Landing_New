...
# -*- coding: utf-8 -*-
"""
Step2: 变速直线运动 — 动态平台降落评估
========================================
船舶以随机变速直线运动 (varspeed), 无人机端到端自主降落。
速度在 [v_min, v_max] 范围内随机切换 (每 2-5s 换一次)。

用法:
  python step2_env.py --test_episodes 100 --ckpt_dir <dir> --load_step 60000 \
      --ship_vx 1.0 --v_min 0.4 --v_max 1.0
"""
...
VEHICLE_TYPE = "iris"
VEHICLE_ID = os.environ.get("STEP2_VEHICLE_ID", "0")
...
def main():
    parser = argparse.ArgumentParser(description="Step2: 变速直线运动 (varspeed) 降落评估")
    # ---- 评估控制 ----
    parser.add_argument('--test_episodes', type=int, default=100,
                        help='评估轮数 (默认 100)')
    # 支持与其他仿真实例隔离运行；默认值保持原有实例 0 配置。
    parser.add_argument('--ros_port', type=int,
                        default=int(os.environ.get('ROS_PORT', '11311')))
    ...
    parser.add_argument('--fcu_url', type=str,
                        default='udp://:24540@localhost:34580')
    ...
    # ==================== 打印配置 ====================
    print("=" * 60)
    print("  Step2 变速直线运动 (varspeed) 降落评估")
    print("=" * 60)
    ...
    print(f"  船方向:         vx={args.ship_vx:.0f} vy={args.ship_vy:.0f}")
    print(f"  速率范围:       [{args.v_min:.1f}, {args.v_max:.1f}] m/s  (随机变速)")
    ...
    try:
        ...
        controller.set_mode_varspeed((args.ship_vx, args.ship_vy),
                                     (args.v_min, args.v_max), args.seed)
        print(f"  ✓ ShipMotionController 就绪: mode=varspeed "
              f"dir=({args.ship_vx:.0f},{args.ship_vy:.0f}) "
              f"range=[{args.v_min:.1f},{args.v_max:.1f}]")
        ...
        print(f"  模型:           {args.ckpt_dir}  step={args.load_step}")
        print(f"  模式:           varspeed  dir=({args.ship_vx:.0f},{args.ship_vy:.0f})  "
              f"range=[{args.v_min:.1f},{args.v_max:.1f}]")
        print("=" * 60)
