...
Step1: 动态平台降落评估脚本
============================
在 step1_linear.launch 仿真环境中, 使用 TD3/LSTM 策略执行多轮降落评估。
船舶以恒定低速直线运动, 无人机端到端自主降落。

与 evaluate_iros_new.py 的区别:
  - 使用 step1_linear.launch (动态平台) 而非 sandisland.launch (静态平台)
  - 船舶由 ShipMotionController 通过 Gazebo 速度伺服按 Step1 直线轨迹移动
  - 每轮结束后船舶瞬移回原点, 无人机 reset (与 evaluate_iros 一致)

用法:
  python step1_env.py --test_episodes 100 --ckpt_dir <dir> --load_step 60000

可选参数:
  --test_episodes N     评估轮数 (默认 100)
  --ship_vx VX          船 x 方向速度 m/s (默认 0.15)
  --ship_vy VY          船 y 方向速度 m/s (默认 0.0)
  --max_steps N         每轮最大步数 (默认 600)
  --seq_len N           LSTM 序列长度 (默认 8, 需与训练一致)
  --ckpt_dir DIR        模型权重目录
  --load_step STEP      加载步数 (默认 60000)

依赖:
  - Simulation/env_base.py (GazeboEnv)
  - Simulation/ship_motion.py (ShipMotionController, Step1 速度伺服)
  - TD3_offline.py (TD3 策略)
  - step1_linear.launch
  - ROS + Gazebo + PX4 SITL + MAVROS + yolov11_ros
"""
...
VEHICLE_TYPE = "iris"
VEHICLE_ID = os.environ.get("STEP1_VEHICLE_ID", "0")
...
def main():
    parser = argparse.ArgumentParser(description="Step1: 动态平台降落评估")
    # ---- 评估控制 ----
    parser.add_argument('--test_episodes', type=int, default=100,
                        help='评估轮数 (默认 100)')
    parser.add_argument('--ros_port', type=int, default=int(os.environ.get('ROS_PORT', '11311')))
    parser.add_argument('--vehicle_id', type=str, default=VEHICLE_ID)
    parser.add_argument('--mavlink_udp_port', type=int, default=18570)
    parser.add_argument('--mavlink_tcp_port', type=int, default=4560)
    parser.add_argument('--fcu_url', type=str, default='udp://:24540@localhost:34580')
    ...
    # ---- 船体运动 ----
    parser.add_argument('--ship_vx', type=float, default=1.0,
                        help='船 x 方向速度 m/s')
    parser.add_argument('--ship_vy', type=float, default=0.0,
                        help='船 y 方向速度 m/s (0=直行)')
    ...
    # ==================== 打印配置 ====================
    ship_speed = np.sqrt(args.ship_vx ** 2 + args.ship_vy ** 2)
    print("=" * 60)
    print("  Step1 动态平台降落评估")
    print("=" * 60)
    ...
    print(f"  船速:           vx={args.ship_vx:.3f} vy={args.ship_vy:.3f} "
          f"合速度={ship_speed:.3f} m/s")
    print(f"  船运动模式:     Step1 速度伺服 (匀速直线), 每轮结束瞬移回原点")
    ...
    try:
        ...
        print("\n[1/4] 启动仿真环境...")
        launch_args = [
            f"ID:={args.vehicle_id}", f"ID_in_group:={args.vehicle_id}",
            f"mavlink_udp_port:={args.mavlink_udp_port}",
            f"mavlink_tcp_port:={args.mavlink_tcp_port}",
            f"fcu_url:={args.fcu_url}", "gui:=false"]
        env = GazeboEnv(LAUNCH_FILE, VEHICLE_TYPE, args.vehicle_id,
                        max_dist=args.max_dist, max_height=args.max_height,
                        ros_port=args.ros_port, launch_args=launch_args)
        print("  ✓ 环境初始化完成")
        ...
        controller.set_mode_constant(vx=args.ship_vx, vy=args.ship_vy)
        print("  ✓ ShipMotionController 就绪: mode=constant (速度伺服, 匀速直线)")
