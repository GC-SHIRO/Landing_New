"""
用途:
- 事件驱动版降落环境, 面向数据采集/训练时对视觉回调同步要求更高的场景。

用法:
- 作为环境模块被 train_listen.py 等脚本导入使用。
- 通过 GazeboEnv.reset()/step() 提供与标准 RL 兼容的交互接口。

实现方式:
- 通过 YOLO 中心点话题回调触发状态更新, 减少固定时间步造成的视觉时延影响。
- 封装 ROS/Gazebo 服务调用、越界与成功判定、奖励计算和环境重置。

依赖关系:
- 依赖 drone.py 中通信与飞控辅助能力。
- 与 train_listen.py 组成专家数据采集链路。
- 依赖 ROS、Gazebo、mavros 与视觉检测节点。
"""
